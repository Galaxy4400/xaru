...
class Tabs {
	constructor(selector, options) {
		...
		this.elements.tabs =
			typeof selector === "object" ? selector : document.querySelector(`[data-tabs="${selector}"]`);
		...
		if (!this.elements.buttons.length) {
			console.error("Элементы закладок должны определяться тегом button");
			return;
		}

		this.elements.containers = this.#getContainers();

		if (this.options.startTabIndex >= this.elements.buttons.length)
			this.options.startTabIndex = this.elements.buttons.length - 1;
	...
	#init() {
		...
		if (this.options.ajax) {
			...
			this.elements.containers.forEach((container) =>
				container.panels.forEach((panel, index) => {
			...
			this.elements.containers.forEach((container) =>
				container.panels[this.options.startTabIndex].classList.add(this.classes.active, this.classes.animate)
			);
		}
	...
	#switchTabs(newTab, oldTab = this.tabs.querySelector("[aria-selected]")) {
		...
		if (!this.options.ajax) {
			this.elements.containers.forEach((container) =>
				container.panels[oldIndex].classList.remove(this.classes.animate)
			);
			setTimeout(() => {
				this.elements.containers.forEach((container) =>
					container.panels[oldIndex].classList.remove(this.classes.active)
				);
				this.elements.containers.forEach((container) =>
					container.panels[index].classList.add(this.classes.active)
				);
				setTimeout(() => {
					this.elements.containers.forEach((container) =>
						container.panels[index].classList.add(this.classes.animate)
					);
				}, 30);
